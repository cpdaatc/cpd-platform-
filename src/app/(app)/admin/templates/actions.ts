...
export async function uploadTemplateVersionAction(formData:FormData){
  ...
  await uploadPrivateDocument({organizationId:c.organizationId,storagePath:path,bytes,contentType:file.type});
  let mapping:unknown=[]; try{mapping=JSON.parse(String(formData.get('mappingJson')??'[]'));}catch{await removePrivateDocument(c.organizationId,path).catch(()=>undefined);throw new Error('Invalid mapping JSON');}
  ...
  if(error){await removePrivateDocument(c.organizationId,path).catch(()=>undefined);throw new Error(error.message);} revalidatePath('/admin/templates');
}
