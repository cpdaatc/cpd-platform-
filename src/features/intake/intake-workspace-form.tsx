...
export function IntakeWorkspaceForm(props: {
  ...
  // Initial keys must be deterministic across server render and browser hydration.
  // Random UUIDs are reserved for explicit client-side "add speaker" interactions below.
  const [speakers, setSpeakers] = useState<Speaker[]>(props.speakers.length ? props.speakers.map((row, index) => ({ clientKey: s(row, 'client_key') || `legacy-speaker-${index + 1}`, fullName: s(row, 'full_name_snapshot'), specialty: s(row, 'specialty_snapshot'), grade: s(row, 'grade_snapshot'), institution: s(row, 'institution_snapshot'), relatedExperiencePastThreeYears: s(row, 'related_experience_past_three_years'), qualificationsSummary: s(row, 'qualifications_summary'), specialCertificatesSummary: s(row, 'special_certificates_summary'), internationalPresentationsCount: row.international_presentations_count == null ? null : Number(row.international_presentations_count), sortOrder: Number(row.sort_order ?? index + 1) })) : [{ clientKey: 'draft-speaker-1', fullName: '', specialty: '', grade: '', institution: '', relatedExperiencePastThreeYears: '', qualificationsSummary: '', specialCertificatesSummary: '', internationalPresentationsCount: null, sortOrder: 1 }]);
