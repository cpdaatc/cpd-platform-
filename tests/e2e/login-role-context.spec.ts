...
test('one multi-role account keeps admin and secretary permissions separated by active role context', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'إدارة الأنشطة', exact: true })).toBeVisible();
  await expect(page.getByRole('link', { name: 'مساحة سكرتير اللجنة', exact: true })).toHaveCount(0);
  ...
  await expect(page.getByRole('link', { name: 'إدارة الأنشطة', exact: true })).toHaveCount(0);
});
